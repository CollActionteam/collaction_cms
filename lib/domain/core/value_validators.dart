...
Either<ValueFailure<String>, String> validateEmailAddress(String input) {
  ...
  if(!input.contains("@collaction.org")) {
    return left(const ValueFailure.incorrectDomain(failureMessage: "Must be a collaction.org email"));
  }
...
Either<ValueFailure<String>, String> validateConfirmPassword(String input, String password) {
  ...
  if(input.isEmpty) {
    return left(const ValueFailure.empty(failureMessage: "Field cannot be empty"));
  }

  if(input != password) {
    return left(
      const ValueFailure.passwordsMismatch(failureMessage: "The passwords don't match")
    );
  }
